...
class BrandApies {
  ...
  getBrandData({String? search, String? categoryID}) async {
    // log(" Admin Token " + SPHelper.spHelper.getAdminToken().toString());
    ...
    try {
      ...
      if (response.statusCode == 200) {
        brandController.getBrandData!.value =
            ListBrandResponse.fromJson(response.data);

        log("getBrandData Successful ");
      } else {}
    } catch (e) {
      log(e.toString());
    }
  ...
  getBrandById({String? brandID}) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        brandController.getBrandByIdData!.value =
            BrandByIdResponse.fromJson(response.data);
        log("getBrandByIdData Successful ");
      } else {}
    } catch (e) {
      log(e.toString());
    }
